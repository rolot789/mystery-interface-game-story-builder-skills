...
작업 페이지의 자동 요약은 `notion_plan.py pages`로 반영하고, 해설은 정본을 읽고 새로 써서 같은 명령으로 넘긴다. 기획서의 세계관 절은 `canon.py bible` 출력을 바탕으로 서술한다. `assets/design-document.md`의 목차로 버전의 세계·인물·진상·조사·분기·미정을 통합하라. 목록을 붙여 넣는 것으로 끝내지 말고 관계와 인과를 읽기 쉬운 문장으로 설명하라. 출처 객체 ID와 리비전을 남기고 새 설정은 추가하지 마라. 완성 판정은 Auditor가 맡는다.
