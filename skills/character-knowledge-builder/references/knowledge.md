...
Claim은 speaker_id, statement, stated_at, audience, intent와 관련 Fact(fact_ids)를 가진다. intent는 truthful / lie / mistaken / uncertain 중 해당하는 설명을 기록한다. LIE는 지식 상태가 아니다. 고의적 거짓말은 당시의 믿음·지식과 주장 간 차이로 설명해야 한다.

acquired_via는 정보가 전달된 Event·Trace·Claim의 ID다. from은 가장 이른 획득 경로의 시각보다 앞설 수 없다(TIME-004). 인물의 앎은 Knowledge로만 기록하고 knows 링크를 쓰지 않는다. 초기부터 아는 사실은 initial_basis로 근거를 설명할 수 있다. 관계가 있다는 사실만으로 서로의 모든 비밀을 안다고 가정하지 않는다.
